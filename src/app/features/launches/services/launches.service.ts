...
export class LaunchesService {
  ...
  getLaunchesList(isUpcoming: boolean = true): Observable<ISimpleLaunch[]> {
    if (isUpcoming) {
      ...
      return this.baseHttpService.get<ISimpleLaunch[]>(`${this.launchesEndPoint}/GetUpcomingLaunches`).pipe(map((res) => {
      ...
      return this.baseHttpService.get<ISimpleLaunch[]>(`${this.launchesEndPoint}/GetPreviousLaunches`).pipe(map((res) => {
        return res.map((value) => {
          return { ...value, launchStatusColor: this.launchUtilService.getBadgeColor(value.status?.abbrev) }
        })
      }));
